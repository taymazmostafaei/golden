...
$(function () {
  ...
  if (select1.length) {
    select1.each(function () {
      var $this = $(this);
      $this.wrap('<div class="position-relative"></div>').select1({
        dropdownParent: $this.parent(),
        placeholder: $this.data('placeholder') //for dynamic placeholder
      });
    });
  ...
  if (dt_category_list_table.length) {
    var dt_category = dt_category_list_table.DataTable({
      ...
      language: {
        sLengthMenu: '_MENU_',
        search: '',
        searchPlaceholder: 'جست و جوی دسته بندی'
      },
      // Button for offcanvas
      buttons: [
        {
          text: '<i class="ti ti-plus ti-xs me-0 me-sm-2"></i><span class="d-none d-sm-inline-block">اضافه کردن دسته بندی</span>',
